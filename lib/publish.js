...
function InitialEntryData (entriesPath, target, title, tags) {
  const workingTitle = deHyphenate(removePostfix(lastPathSegment(title)))
  this.title = workingTitle
  this.path = path.join(entriesPath, title)
  this.href = `./${hyphenate(workingTitle)}.html`
  this.tags = tags || []
}

const makeEntriesPath = (configPath) => {
  const asArray = configPath.split('/')
  const res = path.join(asArray.slice(0, asArray.length - 1).join('/'), './entries')
  return res
}
...
async function publish (opts, ...args) {
  ...
  try {
    ...
    const { target } = (opts.target) 
      ? { target: opts.target } 
      : require(path.join(configPath, 'config.json'))
    ...
    const entriesPath = makeEntriesPath(configPath)
    const files = await readdir(entriesPath)
    const toPublish = (all)
      ? files.filter((x) => x.slice(x.length - 3) === '.md')
             .map((filename) => new InitialEntryData(entriesPath, target, filename))
      : [new InitialEntryData(entriesPath, target, args[0], tags)]
...
const createHtmlFiles = (target, { Page, Index }, { orderedTitles, entries }) => {
  const x = orderedTitles.map((title) => {
    return {
      ...
      html: require('react-dom/server').renderToStaticMarkup(React.createElement(Page, entries[title]))
    }
  })
...
async function publishEntry ({ href, html }) {
  try {
    await writeFile(href, html).then((x) => console.log('success')).catch(console.log)
  } catch (e) {
    throw e
  }
}
